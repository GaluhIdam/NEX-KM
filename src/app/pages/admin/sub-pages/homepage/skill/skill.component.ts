  ...
  implements OnInit, OnDestroy
{
    ...
    this.obs = this.mform.valueChanges
      ...
      .subscribe((data) => {
        if (data.page < 1) {
          ...
          this.getDataSkill(data.page, data.limit, data.search, this.sortBy);
        }
      });
  }
  ngOnDestroy(): void {
  ...
  //Create Skill
  async createDataSkill(): Promise<void> {
    try {
      if (this.submitForm.valid) {
        const dto: InterestSkillCreateDTO = {
          name: this.submitForm.get('title')?.value,
        };
        this.hompageService.createSkill(dto).subscribe(() =>
          Swal.fire({
            icon: 'success',
            title: 'Success!',
            text: 'Skill was successfully created.',
            timer: 1000,
  ...
  //Update Skill
  async updateDataSkill(): Promise<void> {
    try {
      if (this.submitForm.valid) {
        const dto: InterestSkillCreateDTO = {
          name: this.submitForm.get('title')?.value,
        };
        this.hompageService.updateSkill(this.uuid!, dto).subscribe(() =>
          Swal.fire({
            icon: 'success',
            title: 'Success!',
            text: 'Skill was successfully updated.',
            timer: 1000,
            showConfirmButton: false,
          }).then(() => {
            this.submitForm.get('title')?.reset();
  ...
  //Delete Skill
  async deleteDataSkill(uuid: string): Promise<void> {
    try {
      Swal.fire({
        title: 'Are you sure?',
        ...
        if (result.isConfirmed) {
          this.hompageService.deleteSkill(uuid).subscribe(() =>
            Swal.fire({
              icon: 'success',
              title: 'Success!',
              text: 'Skill was successfully deleted.',
              timer: 1000,
              showConfirmButton: false,
            }).then(() => {
              this.submitForm.get('title')?.reset();
