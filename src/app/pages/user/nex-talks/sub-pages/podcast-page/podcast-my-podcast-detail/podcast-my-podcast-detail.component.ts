...
@Component({
  selector: 'app-podcast-my-podcast-detail',
  templateUrl: './podcast-my-podcast-detail.component.html',
  styleUrls: ['./podcast-my-podcast-detail.component.css'],
})
export class PodcastMyPodcastDetailComponent implements OnInit, OnDestroy {
  faPlay = faPlay;
  faPause = faPause;
  ...
  ngOnInit(): void {
    ...
    this.onChangePodcastPage('my-podcast-detail-page');
    this.podcastSubscribe = this.store.select('podcast').subscribe((data) => {
      ...
      this.serieUserList = data.serieUserList;
  ...
  initPodcastListData(refresh?: boolean): void {
    if (this.serieDetailData) {
      ...
      }&status=${true}&serie_id=${this.serieDetailData.id}`;
  ...
  onDeletePodcast(uuid: string) {
    this.isDeletePodcastLoading = true;
    this.podcastDataService.deletePodcastData(uuid).subscribe(
      (response) => {
        this.toastr.success(
          'Selected podcast episode has been removed from the list',
          'Great Move..'
        );
        if (
          this.selectedPodcastToPlay &&
          response.data.id === this.selectedPodcastToPlay.id
        ) {
          this.selectedPodcastToPlay = undefined;
        }
        this.initPodcastListData();
        this.isDeletePodcastLoading = false;
      },
      (error) => {
        this.toastr.error(error.error.message, 'Delete podcast failed');
        this.isDeletePodcastLoading = false;
      }
    );
  }
  ...
  editPodcast(uuid: string): void {
    this.router.navigate(['/user/nex-talks/podcast/my-podcast/edit/' + uuid]);
  }

  editSeries(uuid: string): void {
    this.router.navigate([
      '/user/nex-talks/podcast/my-podcast/edit-series/' + uuid,
    ]);
  }

  onEditSeriesClick(uuid: string) {
    if (this.isStillPlaying) {
      ...
      Swal.fire({
        ...
        text: 'Are you sure want to edit this series?',
        ...
        if (result.isConfirmed) {
          this.editSeries(uuid);
        }
      });
    }
  }

  onEditPodcastClick(row: PodcastDTO) {
    if (this.isStillPlaying) {
      this.podcastPlayingAlert();
    } else {
      Swal.fire({
        icon: 'info',
        title: 'Edit Confirmation',
        html: `<b>Podcast Episode: ${row.title} </b><br>Are you sure want to edit this podcast?`,
        showCancelButton: true,
        confirmButtonColor: '#3085d6',
        cancelButtonColor: ' #d33',
        confirmButtonText: 'Yes',
        ...
        if (result.isConfirmed) {
          this.editPodcast(row.uuid);
        }
      });
    }
  }

  onDeletePodcastClick(row: PodcastDTO) {
    if (this.isStillPlaying) {
      this.podcastPlayingAlert();
    } else {
      Swal.fire({
        ...
        html: `<b>Podcast Episode: ${row.title} </b><br>Are you sure want to delete this podcast?`,
        showCancelButton: true,
        confirmButtonColor: '#3085d6',
        cancelButtonColor: ' #d33',
